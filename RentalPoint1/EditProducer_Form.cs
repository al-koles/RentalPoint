...
{
    public partial class EditProducer_Form : Form
    {
        private int id;
        private bool IsEdit;
        public EditProducer_Form()
        {
            InitializeComponent();
            ...
            id = Convert.ToInt32(producerTableAdapter.TheLastID()) + 1;
            this.ProducerID_textBox.Text = id.ToString();
            IsEdit = false;
        }
        public EditProducer_Form(int id) : this()
        {
            var rows = producerTableAdapter.WhereId(id);
            var row = rows[0].ItemArray;
            IsEdit = true;
            this.id = id;
            this.ProducerID_textBox.Text = id.ToString();
            this.ProducerName_textBox.Text = row[1].ToString();
        }
        ...
        private void Accept_button_Click(object sender, EventArgs e)
        {
            ...
            //{
                if (IsEdit)
                {
                    try
                    {
                        producerTableAdapter.UpdateQuery(id, ProducerName_textBox.Text, id);
                    }
                    catch (Exception ex)
                    {
                ...
                else
                {
                    try
                    {
                        producerTableAdapter.Insert(id, ProducerName_textBox.Text);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message);
        ...
        private void EditProducerForm_Load(object sender, EventArgs e)
        {
